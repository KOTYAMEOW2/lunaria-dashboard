...
async function handleCallbackIfAny(supabase) {
  ...
  // Callback handler:
// - Implicit grant: tokens in URL hash (#access_token=...&refresh_token=...)
// - PKCE/code flow: ?code=...
  ...
  try {
    ...
    if (location.hash && location.hash.includes("access_token")) {
      ...
      if (!access_token || !refresh_token) {
        throw new Error("Callback hash missing tokens (access_token/refresh_token).");
      }
    ...
    showError("Ошибка авторизации: " + (e?.message || String(e)));
    console.error(e);
    return false;
  }
  return false;
